...
物差しが要る。**差分と DOM 要素は結びつけない**。変わった行そのものに幅ゼロの
目印 (`.ink-at`) を挿し、行頭は行の上端・行末は下端に揃えてあるので、2 つ
合わせればその行が画面で占める高さがそのまま出る。折り返しても画像が混ざっても合う。
...
線・左右を結ぶ帯・スクロールの対応点は、すべて `changeSpans()` / `changeRuns()`
から出す。別々に組み立てると、まとめ方の違いがそのままズレになって出る。
