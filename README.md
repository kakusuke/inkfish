...
フォルダが git のリポジトリなら、ツリーの下に変更のペインが出ます。
リポジトリでなければ何も出ず、ツリーの見た目も変わりません。

ツリーのファイル名は **HEAD からの差分**で色が変わります(変更・追加・
コンフリクト)。ディレクトリは子孫の状態をまとめて示すので、畳んでいても
中に変更があることが分かります。

ペインには**選んだ範囲**で変わった Markdown が並びます。ツリーが
「まだコミットしていない自分の編集」を示すのに対して、こちらは範囲全体を
見るためのもので、削除されたファイルもここに出ます(押して開くことはできません)。
Markdown 以外の変更はファイル数だけ添えます。

範囲は「どこまで」と「どこから」で決めます。

| | |
|---|---|
| どこまで | 作業ツリー / ステージ済み / HEAD / 任意のブランチ・タグ |
| どこから | 分岐点(merge-base)/ 任意のブランチ・タグ・リビジョン |

既定は「分岐元の分岐点 → 作業ツリー」、つまりそのブランチで書いた分が全部出ます。
`HEAD → 作業ツリー` にすれば未コミットの変更だけ、`main → develop` のように
ブランチ同士を比べることもできます。「どこから」の分岐点の候補には、
**まだ「どこまで」に取り込まれていないブランチ**だけが並びます(取り込み済みの
ブランチは分岐点がそのブランチ自身になり、そのまま指定したときと同じ結果に
なるためです)。範囲はフォルダごとに覚えます。

`HEAD~3` のようなリビジョンの指定も直接書けます。fetch はしないので、
リモートのブランチは最後に取得した時点のものです(いつ取得したかは候補の
見出しに出ます)。

### 差分を読む

変更ペインの行を押すと、起点版と終点版が左右に並びます。変わったところは
脇に線が付き(変更はオレンジ、追加は緑、削除は赤)、左右のスクロールは
連動します。⌘⌥↓ / ⌘⌥↑ で変わったところを渡り歩けます。

行を右クリックすると、差分・変更前・変更後を選んで開けます。差分のタブも
切り離せて、単一文書ウィンドウのまま左右に並びます。

git の操作には [gitoxide](https://github.com/GitoxideLabs/gitoxide) を使っていて、
`git` コマンドには依存しません。
...
ウィンドウ管理のみを担い、Tauri の IPC で連携します。git の読み取りだけは
`src-tauri/src/git.rs` に分けてあります。
